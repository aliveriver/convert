...
- **格式互转无需 AI**：Word / Markdown / LaTeX 三种格式直接互转，基于解析器和渲染器完成，速度快、零成本。
...
- `frontend/` - 前端 Web 界面代码（`index.html`、`compare.html`、`format_convert.html`）
- `word_agent/` - Python 核心包：
  ...
  - `format_convert.py` - 格式互转核心逻辑（Parser → Renderer 直接转换）
